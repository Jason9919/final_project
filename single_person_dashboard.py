...
# 필요한 열만 선택하여 불러오기
df_1 = df_1[['2014년_1인세대', '2015년_1인세대', '2016년_1인세대', '2017년_1인세대', '2018년_1인세대',
             '2019년_1인세대', '2020년_1인세대', '2021년_1인세대', '2022년_1인세대', '2023년_1인세대',
             '2014년_전체세대', '2015년_전체세대', '2016년_전체세대', '2017년_전체세대', '2018년_전체세대',
...
st.title("1인 세대 수 데이터 대시보드")
st.sidebar.title("옵션 설정")

# 연도 선택 사이드 바 만들기
available_years = [col.split("_")[0] for col in gdf_gu.columns if "1인세대" in col]
selected_year = st.sidebar.selectbox("연도 선택", sorted(available_years))
data_column = f"{selected_year}_1인세대"
...
colormap.caption = f"{selected_year} 1인 세대 수"
...
donut_fig = px.pie(
    ...
    values=data_column,
    names='행정시',
    title=f"{selected_year} 1인세대 분포 (행정시 기준)",
    hole=0.4,
...
if selected_year_int > 2014: 
    ...
    prev_data_column = f"{prev_year}년_1인세대"
...
## 가장 처음으로 선택된 행정구에 대한 1인세대 증감률을 보여줌
    if selected_gu:  
        ...
        if prev_data_column in gdf_gu.columns:
            ...
            growth_rate = None
            change_amount = None
        target_label = f"{first_selected_gu}의 {selected_year} 1인세대 증가율"
...
# 선택된 행정구가 없으면 전체 데이터를 기준으로 증감률을 계산함
        ...
        if prev_data_column in gdf_gu.columns:
            ...
            growth_rate = None
            change_amount = None
        target_label = f"{selected_year} 전체 1인세대 증가율"
else:
    growth_rate = None
    change_amount = None
